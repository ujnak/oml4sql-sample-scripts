...
-- the random forest regression algorithm.
  ...
  sys.rqScriptCreate('RF_RDEMO_BUILD_REGRESSION', 'function(dat, form) {
   ...
   set.seed(1234); 
   mod <- randomForest(formula = formula(form), data=dat, na.action=na.omit); 
   mod}');
...
BEGIN
  ...
  v_setlst('RALG_DETAILS_FUNCTION') := 'RF_RDEMO_DETAILS_REGRESSION';
...
begin
  ...
  v_setlst('RALG_DETAILS_FUNCTION') := 'RF_RDEMO_DETAILS_CLASSIFICATION';
  v_setlst('RALG_WEIGHT_FUNCTION')  := 'RF_RDEMO_WEIGHT_CLASSIFICATION';
